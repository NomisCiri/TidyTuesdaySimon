...
#plots<-list()
  ...
  scale_fill_continuous_divergingx(name = "stress rating", label = scales::comma,limits=c(unique(.x$min),unique(.x$max)),mid=0,palette = "Spectral") + 
  #facet_wrap(stressor)+
...
```{r}
cowplot::plot_grid(plots1[[1]]+theme(legend.position = "none"), 
                   ...
                   plots1[[2]]+theme(legend.position = "none"),get_legend(plots1[[2]]),
                   rel_widths = c(1,1,1,1,1,1,0.3),
                   rel_heights = c(1,1,0.2),nrow=3)
```
